...
function resultFn(data) {
	...
	for (var i = 0; i < data.result.length; i++) {
		...
		$('.prd_out_wrap').imagesLoaded().done(function (instance) {
			$(".prd_out_wrap").css({
				"height": $(".prd_wrap").eq(0).height() + "px"
			});
		})
	}
	//생성완료된 후 이벤트 처리
	$(".prd_nav > li").click(function () {
		$(".prd_out_wrap").css({
			"height": $(".prd_wrap").eq(prdNum).height() + "px"
		});
	...
	$(".prd").hover(function () {
		...
		$(this).find(".prd_compare").find("div").stop().animate({
			"top": "-43px"
		}, 300);
		if ($(this).find(".prd_cont")[0].offsetHeight < $(this).find(".prd_cont")[0].scrollHeight) {
			console.log("overflow");
			$(this).find(".prd_cont").children("div").stop().animate({
				"bottom": 0
			}, 200);
		...
		$(this).find(".prd_detail").children("ul").hover(function () {
			$(this).children(":first-child").stop().animate({
				"margin-top": "-38px"
			}, 200);
		}, function () {
			$(this).children(":first-child").stop().animate({
	...
	$(".prd_hover_img").hover(function () {
		$(this).stop().animate({
			...
			"animation-name": "prdImg"
		});
	}, function () {
		$(this).stop().animate({
			"opacity": 0
		}, 200).css({
			"animation-name": "prdImgBack"
		});
	});
...
function fbanPos() {
	...
	$(".fban > li").eq(fNum).show().css({
		"left": 0
	});
	if (fNum == 0) {
		$(".fban > li").eq(fLen).show().css({
			"left": "-100%"
		});
		$(".fban > li").eq(1).show().css({
			"left": "100%"
		});
	} else if (fNum == fLen) {
		$(".fban > li").eq(fNum - 1).show().css({
			"left": "-100%"
		});
		$(".fban > li").eq(0).show().css({
			"left": "100%"
		});
	} else {
		$(".fban > li").eq(fNum - 1).show().css({
